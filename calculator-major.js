...
const sqrt = (n1)=> (n1 >= 0 ? Math.sqrt(n1): "Square root of negative number is not allowed");
const mod = (n1,n2)=> (n2 !==0 ? n1%n2: "Modulus by zero is not allowed");
...
app.get("/add", (req,res)=>{
    ...
    app.get("/exp", (req,res)=>{
        try{
            const n1=parseFloat(req.query.n1);
            const n2=parseFloat(req.query.n2);
            logger.info('New exponentiation operation requested: '+n1+' ^ '+n2+' parameters');
            if(isNaN(n1)) {
                logger.error("Invalid value for n1");
                throw new Error("Invalid value for n1");
            }
            ...
            logger.info('Exponentiation Operation Parameters '+n1+' ^ '+n2+' is received');
    ...
    app.get("/sqrt", (req,res)=>{
        try{
            const n1=parseFloat(req.query.n1);
            logger.info('New square root operation requested: '+n1+' parameters');
            if(isNaN(n1)) {
                logger.error("Invalid value for n1");
                throw new Error("Invalid value for n1");
            }
            ...
            logger.info('Square Root Operation Parameters '+n1+'  is received');
        ...
        app.get("/mod", (req,res)=>{
            try{
                const n1=parseFloat(req.query.n1);
                const n2=parseFloat(req.query.n2);
                logger.info('New modulo operation requested: '+n1+' and '+n2+' parameters');
                if(isNaN(n1)) {
                    logger.error("Invalid value for n1");
                    throw new Error("Invalid value for n1");
                }
                ...
                logger.info('Modulo Operation Parameters '+n1+' and '+n2+' is received');
